# stapel-calendar — contract emission + drift gate (contract-pipeline.md §2-3).
#
# This module emits its OWN contract triad (schema.json + flows.json + errors.json)
# per-module, from a single-module {calendar + core} Django instance mounted at
# the canonical /calendar/api/ prefix (see _codegen.py / _codegen_settings.py /
# codegen_urls.py).
#
